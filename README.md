...
- **BOLT** goes long after **two consecutive completed 15-minute closes** above a recent 8-bar resistance, **or** one strong breakout bar with a volume spike (≥1.25× lookback average) / ATR range expansion. On the volume-spike / ATR-expansion path only, last may clear resistance from 1×ATR below (`last >= res − ATR`); the two-close path stays strict. Weak volume no longer blocks the two-close path. Size is capped at ~5.5% of equity (hard cap 6%), with a ~1% hard stop, partial take near +1% (stop to breakeven), and a ~2% target. Each skip is logged (resistance, last closes, volume, ATR) for daily research.
- Fair value (EMA vs SMA) still drives ILSA / KETT. TESS stays in the grid but starts paused so BTC/ETH drive the book (smaller size if manually resumed). Prediction modules stay paused — Polymarket is owned by another agent.
